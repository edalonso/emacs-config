...
(setf bbdb-default-country "Spain"
...
#+BEGIN_SRC emacs-lisp
(setf user-full-name "Eduardo Alonso")
(setf user-mail-address "eduardo.alonso@disashop.com")
...
;;* Key Bindings
;;here are different types of key bindings. Most of them are related to the
;;ame of the underlying function, e.g. `C-x v' for version control, or the
;;ey `b' for buffer related activities. But there are also bindings that
;;epend only on the physical position of the keys on the keyboard. This is
;;he case for vim-style motion keys and some key chords. However a single
;;eyboard can be used with different layouts, like QWERTY or the German Neo
;;ayout. This configuration permits to select different positional bindings
;;o accommodate for different keyboard layouts.
;;
;;+BEGIN_SRC emacs-lisp
...
;;+END_SRC
;;
;;mprove evil interaction of several major modes.
;;
;;+BEGIN_SRC emacs-lisp
...
;;+END_SRC
;;
;;se occur to search one or more buffers.
;;
;;+BEGIN_SRC emacs-lisp
...
;;here are some modes that are so omnipresent that they deserve no special
;;ention in the [[info:Emacs#Mode%20Line][Mode Line]]. The small package `diminish' gets rid of them.
;;
;;+BEGIN_SRC emacs-lisp
